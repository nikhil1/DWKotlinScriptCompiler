...
public class CodeGenerator {
    ...
    private VariableEntry getEntryFor(final Variable variable) throws CodeGeneratorException {
        ...
        if (entry != null) {
            return entry;
        } else if(variable instanceof ArrayWithIndexExp exp) {
            VariableEntry arrayEntry = variables.get(exp.getVariableExp());
    ...
    private VariableEntry addEntry(final Variable variable, final Type type) throws CodeGeneratorException {
        if (variables.containsKey(variable)) {
            ...
            if(variables.containsKey(exp.getVariableExp())) {
                throw new CodeGeneratorException("Should not add ArrayWithIndexExp into entry.");
            } else {
                throw new CodeGeneratorException("Array is not in scope: " + exp.getVariableExp());
            }
        } else {
    ...
    private void writeBiLogicalExp(BiLogicalExp exp) throws CodeGeneratorException {
        ...
        switch (exp.getOp()) {
            case OP_AND:
                if(left instanceof BooleanExp booleanExp) {
                    writeIntLiteral(booleanExp.getValue() ? 1 : 0);
                } else if(left instanceof ComparableExp comparableExp) {
                    writeComparableExp(comparableExp);
                } else if(left instanceof BiLogicalExp logicalExp) {
                    writeBiLogicalExp(logicalExp);
                } else if(left instanceof NotExp notExp) {
                    writeNotExp(notExp);
                } else if(left instanceof Variable variable) {
                ...
                if(right instanceof BooleanExp booleanExp) {
                    writeIntLiteral(booleanExp.getValue() ? 1 : 0);
                } else if(right instanceof ComparableExp comparableExp) {
                    writeComparableExp(comparableExp);
                } else if(right instanceof BiLogicalExp logicalExp) {
                    writeBiLogicalExp(logicalExp);
                } else if(right instanceof NotExp notExp) {
                    writeNotExp(notExp);
                } else if(right instanceof Variable variable) {
                    loadVariable(variable);
                }else {
    ...
    private void writeForStatement(final ForStmt forStmt) throws CodeGeneratorException {
        ...
        if(forStmt.getArrayExp() != null) {
            ...
            if(type instanceof TypeArray array) {
                basicType = array.getBasicType();
            } else if(type instanceof TypeMutableList list) {
                basicType = list.getBasicType();
            } else {
        ...
        if(blockStmt != null) {
            for (Stmt s : blockStmt.getStmtList()) {
                if (s instanceof ControlLoopStmt) {
                    ...
                    writeIfStatementInLoop(stmt, head, beforeInc, afterLoop);
                } else {
                    writeStatement(s);
                }
            }
    ...
    private void writeStatement(final Stmt stmt) throws CodeGeneratorException {
        if (stmt instanceof VariableDeclareStmt) {
        ...
        } else if (stmt instanceof AssignStmt asAssign) {
            ...
            if(asAssign.isNew()) {
                ...
                entry = getEntryFor(asAssign.getVariable());
                if(entry.variable instanceof ArrayWithIndexExp exp) {
                    methodVisitor.visitVarInsn(ALOAD, entry.index);
                    writeExp(exp.getIndexExp());
                    writeExp(asAssign.getExpression());
                } else {
                    writeExp(asAssign.getExpression());
                }
            ...
            entry.store(this, methodVisitor);
        } else if(stmt instanceof CompoundAssignStmt asAssign) {
            ...
            if(type == BasicType.TYPE_STRING) {
                isStringAppend = true;
            } else if(type instanceof TypeArray array && asAssign.getVariable() instanceof ArrayWithIndexExp && array.getBasicType() == BasicType.TYPE_STRING) {
                isStringAppend = true;
            } else {
                entry.load(this, methodVisitor);
            }

            switch (asAssign.getOp()) {
                case EXP_PLUS_EQUAL:
                    if(isStringAppend) {
            ...
            if(stmt instanceof PrintStmt printStmt) {
                writePrint( printStmt.getValue(), false);
            } else {
            ...
            writeExp(returnStmt.getReturnExp());
        } else if(stmt instanceof BlockStmt blockStmt) {
            writeBlockStmt(blockStmt);
        } else if(stmt instanceof FunctionInstanceStmt instanceStmt) {
            writeFunctionInstance(instanceStmt.getFunctionInstanceExp());
        } else if(stmt instanceof ForStmt forStmt) {
            writeForStatement(forStmt);
        } else if(stmt instanceof SelfOperationStmt operationStmt) {
            writeSelfOperationExp(operationStmt.getSelfOperationExp(), false);
        }
        else {
//            assert(false);
    ...
    private Type typeOf(Exp temp) throws CodeGeneratorException {
        ...
        if(temp instanceof IntExp || temp instanceof SelfOperationExp) {
            ...
            if(temp instanceof AdditiveExp exp) {
                Type left = typeOf(exp.getLeft());
                if(exp.getOp() == AdditiveOp.EXP_PLUS) {
        ...
        } else if(temp instanceof FunctionInstanceExp exp) {
            type = typeOfFunctionInstance(exp);
        } else if(temp instanceof ArrayOfExp exp) {
            if(exp.getExpList().size() > 0) {
                Exp t = exp.getExpList().get(0);
        ...
        } else if(temp instanceof ArrayExp exp) {
            Type genericType = typeOf(exp.getLambdaExp().getReturnExp());
    ...
    public Type writeExp(Exp exp) throws CodeGeneratorException {
        if(exp == null) {
            // Do nothing
            return null;
        } else if(exp instanceof IntExp intExp) {
            writeIntLiteral(intExp.getValue());
            return BasicType.TYPE_INT;
        } else if(exp instanceof BooleanExp booleanExp) {
            writeIntLiteral(booleanExp.getValue() ? 1 : 0);
            return BasicType.TYPE_BOOLEAN;
        } else if(exp instanceof StringExp stringExp) {
            writeStringExp(stringExp);
            return BasicType.TYPE_STRING;
        } else if(exp instanceof VariableExp variableExp) {
            return loadVariable(variableExp).type;
        } else if(exp instanceof ArrayWithIndexExp indexExp) {
            return loadVariable(indexExp).type;
        } else if(exp instanceof SelfOperationExp operationExp) {
            writeSelfOperationExp(operationExp, true);
            return BasicType.TYPE_INT;
        } else if(exp instanceof BinaryIntExp intExp) {
            if(writeAdditiveExpOrMultplicativeExp(intExp)) {
                methodVisitor.visitMethodInsn(INVOKEVIRTUAL, "java/lang/StringBuilder", "toString", "()Ljava/lang/String;", false);
            }
            return typeOf(exp);
        } else if(exp instanceof ComparableExp comparableExp) {
            writeComparableExp(comparableExp);
            return BasicType.TYPE_BOOLEAN;
        } else if(exp instanceof NotExp notExp) {
            writeNotExp(notExp);
            return BasicType.TYPE_BOOLEAN;
        } else if(exp instanceof BiLogicalExp logicalExp) {
            writeBiLogicalExp(logicalExp);
            return BasicType.TYPE_BOOLEAN;
        } else if(exp instanceof FunctionInstanceExp instanceExp) {
            return writeFunctionInstance(instanceExp);
        } else if(exp instanceof ArrayOfExp ofExp) {
            if(ofExp.getExpList().size() > 0) {
                Exp temp = ofExp.getExpList().get(0);
                BasicType type = (BasicType) typeOf(temp);
                boolean isAny = false;
                for (Exp e : ofExp.getExpList()) {
                    if(type != typeOf(e)) {
                        isAny = true;
                        break;
                    }
                ...
                writeValueToArray(type, ofExp.getExpList());
                return new TypeArray(type);
            } else {
                throw new CodeGeneratorException("arrayOf(exp*) should has at least one parameter");
            }
        } else if(exp instanceof ArrayExp arrayExp) {
            Type returnGenericType = typeOf(arrayExp.getLambdaExp().getReturnExp());
            writeArrayExp(arrayExp);
    ...
    private boolean writeAdditiveExpOrMultplicativeExp(BinaryIntExp exp) throws CodeGeneratorException {
        ...
        Exp right = exp.getRight();
        boolean isStringAppend = false;
        if(left instanceof IntExp intExp) {
            writeIntLiteral(intExp.getValue());
        } else if(left instanceof VariableExp variableExp) {
            final VariableEntry entry = getEntryFor(variableExp);
            if(entry.type == BasicType.TYPE_INT) {
                entry.load(this, methodVisitor);
        ...
        } else if(left instanceof StringExp stringExp) {
            isStringAppend = true;
            methodVisitor.visitTypeInsn(NEW, "java/lang/StringBuilder");
            methodVisitor.visitInsn(DUP);
            methodVisitor.visitMethodInsn(INVOKESPECIAL, "java/lang/StringBuilder", "<init>", "()V", false);
            writeStringExp(stringExp);
            methodVisitor.visitMethodInsn(INVOKEVIRTUAL, "java/lang/StringBuilder", "append", "(Ljava/lang/String;)Ljava/lang/StringBuilder;", false);
        } else if(left instanceof BinaryIntExp intExp) {
            isStringAppend = writeAdditiveExpOrMultplicativeExp(intExp);
        } else if(left instanceof SelfOperationExp operationExp) {
            writeSelfOperationExp(operationExp, true);
        } else if(left instanceof ArrayWithIndexExp indexExp) {
            final VariableEntry entry = getEntryFor(indexExp);
        ...
        if(right instanceof IntExp intExp) {
            if(isStringAppend) {
                writeIntLiteral(intExp.getValue());
        ...
        } else if(right instanceof VariableExp variableExp) {
            final VariableEntry entry = getEntryFor(variableExp);
            if(entry.type == BasicType.TYPE_INT) {
                if(isStringAppend) {
                    entry.load(this, methodVisitor);
        ...
        } else if(right instanceof BinaryIntExp intExp) {
            writeAdditiveExpOrMultplicativeExp(intExp);
        } else if(right instanceof SelfOperationExp operationExp) {
            if(isStringAppend) {
                writeSelfOperationExp(operationExp, true);
                methodVisitor.visitMethodInsn(INVOKEVIRTUAL, "java/lang/StringBuilder", "append", "(I)Ljava/lang/StringBuilder;", false);
            } else {
        ...
        } else if(right instanceof BooleanExp booleanExp) {
            if(isStringAppend) {
                writeIntLiteral(booleanExp.getValue() ? 1 : 0);
                methodVisitor.visitMethodInsn(INVOKEVIRTUAL, "java/lang/StringBuilder", "append", "(Z)Ljava/lang/StringBuilder;", false);
            } else {
        ...
        else if(right instanceof ArrayWithIndexExp indexExp) {
            final VariableEntry entry = getEntryFor(indexExp);
            if(isStringAppend) {
                BasicType basicType = ((TypeArray) entry.type).getBasicType();
                entry.load(this, methodVisitor);
        ...
        } else if(right instanceof StringExp stringExp) {
            if(isStringAppend) {
                writeStringExp(stringExp);
                methodVisitor.visitMethodInsn(INVOKEVIRTUAL, "java/lang/StringBuilder", "append", "(Ljava/lang/String;)Ljava/lang/StringBuilder;", false);
            } else {
        ...
        if(exp instanceof AdditiveExp additiveExp) {
            AdditiveOp op = additiveExp.getOp();
        ...
        } else if(exp instanceof MultiplicativeExp multiplicativeExp) {
            if(isStringAppend) {
                throw new CodeGeneratorException("String concatenation is only allow in +");
            }
            MultiplicativeOp op = multiplicativeExp.getOp();
    ...
    private void loadFunctionTable(final Program program) throws CodeGeneratorException {
        for (final Stmt s : program.getStmtList()) {
            if(s instanceof FunctionDeclareStmt stmt) {
                if (functionTable.containsKey(stmt.getFuncName().getName())) {
                    throw new CodeGeneratorException("Duplicate function name: " + stmt.getFuncName().getName());
                }
                functionTable.put(stmt.getFuncName().getName(), stmt);
            }
        }
    } // loadFunctionTable
