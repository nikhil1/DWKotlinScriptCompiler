...
//        int curPos = startPos;
    ...
    private ParseResult<Exp> parsePrimary(final int startPos) throws ParseException {
        ...
        if (tokenHere instanceof VariableToken asVar) {
            if(startPos + 1 < tokens.length) {  // we dont want throw exception now.
                ...
                if(next == BracketsToken.TK_LPAREN) {
                    ...
                    while((temp = readToken(pos)) != BracketsToken.TK_RPAREN) {
                        if(temp instanceof VariableToken token) {
                            parameterList.add(new VariableExp(token.getName()));
                        } else if(temp instanceof IntToken token) {
                            parameterList.add(new IntExp(token.getValue()));
            ...
            return new ParseResult<>(new VariableExp(asVar.getName()), startPos + 1);
        } else if(tokenHere instanceof IntToken asInt) {
            ...
            if(startPos + 1 < tokens.length) {
                Token next = readToken(startPos + 1);
                if(next instanceof VariableToken token) {
                    if(startPos + 2 < tokens.length) {
                        ...
                        if(readToken(pos) == BracketsToken.TK_LBRACKET) {   // array with index case
                            ...
                            checkTokenIs(pos, BracketsToken.TK_RBRACKET);
                            pos++;
                            return new ParseResult<>(new SelfOperationExp(new ArrayWithIndexExp(new VariableExp(token.getName()), result.result),
                                    tokenHere == UnopToken.TK_PLUS_PLUS ? SelfOp.OP_SELF_INCREASE : SelfOp.OP_SELF_DECREASE,
                                    true), pos);
                        }
                    }
                    return new ParseResult<>(new SelfOperationExp(new VariableExp(token.getName()),
                            tokenHere == UnopToken.TK_PLUS_PLUS ? SelfOp.OP_SELF_INCREASE : SelfOp.OP_SELF_DECREASE,
                            true), startPos + 2);
                } else {
...
//                        location++;
    ...
    private ParseResult<Exp> parseLambdaExp(final int startPos) throws ParseException {
        ...
        while((tokenHere = readToken(pos)) != SymbolToken.TK_ARROW) {
            if (tokenHere instanceof VariableToken token) {
                variableExp = new VariableExp(token.getName());
                pos++;
    ...
    private ParseResult<Stmt> parsePrimaryStmt(final int startPos) throws ParseException {
        ...
        if(tokenHere instanceof VariableToken asVar) {
            ...
            if(startPos + 1 < tokens.length) {
                ...
                if(next instanceof VariableToken token) {
                    if(startPos + 2 == tokens.length) {
                        stmtResult = new ParseResult<>(new SelfOperationStmt(new SelfOperationExp(new VariableExp(token.getName()),
...
//                        Token temp;
                        if(readToken(pos) == BracketsToken.TK_LBRACKET) {
                            ...
                            pos++;
                            if(pos == tokens.length) {
                                stmtResult = new ParseResult<>(new SelfOperationStmt(new SelfOperationExp(new ArrayWithIndexExp(new VariableExp(token.getName()), result.result),
                                        tokenHere == UnopToken.TK_PLUS_PLUS ? SelfOp.OP_SELF_INCREASE : SelfOp.OP_SELF_DECREASE,
                                        true)), pos);
                            } else {
                                checkTokenIsOr(pos, SymbolToken.TK_LINE_BREAK, SymbolToken.TK_SEMICOLON);
            ...
            Token next = readToken(startPos + 1);
            Type type = null;
            if(next instanceof VariableToken asVar) {
                int pos = startPos + 2;
