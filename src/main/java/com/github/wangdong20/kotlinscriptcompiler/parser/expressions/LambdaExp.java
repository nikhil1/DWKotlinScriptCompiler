...
public class LambdaExp implements Exp {
    ...
    public boolean equals(Object obj) {
        if(obj instanceof LambdaExp exp) {
            if(exp.getReturnExp().equals(returnExp)) {
                if ((exp.getParameterList() == null && parameterList == null)
                        || exp.getParameterList().equals(parameterList)
                    || (exp.getParameterList().size() == 0 && parameterList == null)
                    || (exp.getParameterList() == null && parameterList.size() == 0)){
                    return true;
                }
            }
        }
